...
if __name__ == "__main__":
    ...
    # Create the parser
    vq_parser = argparse.ArgumentParser(description='Image generation using VQGAN+CLIP',formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    # Add the arguments
    ...
    vq_parser.add_argument("-conf", "--vqgan_config", type=str, help="VQGAN config", default=f'checkpoints/vqgan_imagenet_f16_1024.yaml', dest='vqgan_config')
    vq_parser.add_argument("-ckpt", "--vqgan_checkpoint", type=str, help="VQGAN checkpoint", default=f'checkpoints/vqgan_imagenet_f16_1024.ckpt', dest='vqgan_checkpoint')
    ...
    vq_parser.add_argument("-ofps", "--output_video_fps", type=float, help="Create an interpolated video (Nvidia GPU only) with this fps (min 10. best set to 30 or 60)", default=10, dest='output_video_fps')
    ...
    if args.init_image:
        ...
        pil_image = img.convert('RGB')
        pil_image = pil_image.resize((sideX, sideY), Image.NEAREST)
        pil_tensor = TF.to_tensor(pil_image)
        z, *_ = model.encode(pil_tensor.to(device).unsqueeze(0) * 2 - 1)
    else:
    ...
    def checkin(i, losses):
        ...
        #image = image.resize((1200,800), PIL.Image.NEAREST)
        #image.save(args.output, optimize=True, quality=120)
        ...
        if args.output_video_fps > 9:
            ...
            try:
                ...
                p = Popen(['ffmpeg',
                           ...
                           '-preset', 'veryslow',
                           '-metadata', f'comment={args.prompts}',
                           output_file,'-loglevel error'], stdin=PIPE)
            except FileNotFoundError:
                print("ffmpeg command failed - check your installation")
            for im in tqdm(frames):
                im.save(p.stdin, 'PNG')
